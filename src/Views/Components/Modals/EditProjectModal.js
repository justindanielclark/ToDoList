...
const editProjectModal = (root, controller, project) => {
  //*Closure Utility Variables
  let _chosenColor = project.getColor();
  let _chosenIconName = project.getIconName();
  let _chosenIcon; //currently undefined, will be found on Dom Creation once IconMap is iterated through
  ...
  const _modalTitle = document.createElement('h1');
    _modalTitle.innerText = `Edit Project: ${project.getName()}`;
  ...
  const _nameInputGroupTextInput = document.createElement('input');
    ...
    _nameInputGroupTextInput.value = `${project.getName()}`
  ...
  for(let icon in IconMap){
    ...
    if(icon === _chosenIconName){
      _handleClick_IconLabel(null, IconLabel);
    }
  ...
  function _handleClick_AcceptButton(event){
    ...
    if(!projectName || !_chosenIcon){
      ...
      controller.publish('editProject', {
